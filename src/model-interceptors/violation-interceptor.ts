...
export class ViolationInterceptor implements ModelInterceptorContract<Violation> {
  ...
  receive(model: Violation): Violation {
    ...
    model.competentProsecutionDecisionInfo = AdminResult.createInstance(model.competentProsecutionDecisionInfo);
    model.courtDecisionInfo = AdminResult.createInstance(model.courtDecisionInfo);
    model.drugProsecutionDecisionInfo = AdminResult.createInstance(model.drugProsecutionDecisionInfo);
    model.forensicLabAnalysisInfo = AdminResult.createInstance(model.forensicLabAnalysisInfo);
    model.securityAdminDecisionInfo = AdminResult.createInstance(model.securityAdminDecisionInfo);
    return model;
  }
}
