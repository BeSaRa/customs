...
export class UserTeamService extends BaseCrudWithDialogService<UserTeamPopupComponent, UserTeam> {
  ...
  loadUserTeams(
    ...
    return this.http.get<Pagination<UserTeam[]>>(this.getUrlSegment() + `/${userId}`, {
  ...
  saveUserTeam(userTeam: { internalUserId: number; teamId: number; status: number }): Observable<unknown> {
    return this.http.post(this.teamService.getUrlSegment() + '/admin/add-user', userTeam);
  }

  override delete(id: number): Observable<UserTeam> {
    return this.http.delete<UserTeam>(this.teamService.getUrlSegment() + '/admin/remove-users/bulk', { body: [id] });
  }
}
